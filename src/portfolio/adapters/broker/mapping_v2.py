...
def _cash_operation_row(
    ...
    yield ParsedOperation(
        ...
        # Купон и дивиденд показаны до удержания (A-04). У купона это проверено
        # прямо: выплата равна «количество × размер купона», то есть из неё
        # ничего не вычтено, и строки налога по купонам в разделе 8.1.1 нет
        # вовсе. У дивиденда налог приходит отдельной строкой — сумма выплаты
        # тоже полная.
