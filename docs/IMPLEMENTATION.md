...
| T4 Схема | 4 таблицы, полная схема журнала, UNIQUE на ключе | `models.py`, `migrations/versions/0001_stage1_journal.py`, `0002_fee_kind_stamp.py` |
...
| Шаг 0 Разведка архива | форма отчёта разобрана по 15 обезличенным отчётам 2017–2020 и выжимке с 14 отчётов 2024–2026 | `docs/BROKER-REPORT-FORMAT.md`, `tools/report_structure.py` |
| Разбор реального формата | нарезка плоской выгрузки Excel на разделы, стадия 2 под неё, выбор версии по форме документа | `adapters/broker/{sections,mapping_v2,mapping}.py` |

Не выполнено: вторая половина T11 — прогон по реальному архиву и разбор того, что
он покажет. Следствия описаны в разделе 11.
...
значений мало. Так добавлен `FeeKind.STAMP` (гербовый сбор — отдельная колонка
реального отчёта): миграция `0002_fee_kind_stamp` пересоздаёт CHECK, а не меняет
тип. Её откат сначала удаляет записи со `STAMP` — перевести их в `OTHER` значило
бы молча смешать их с прочими комиссиями.
...
| `init-db` | `jobs.init_db.init_db` | `2`, если `DATABASE_URL` не SQLite |
...
`COLUMNS_V2` — отдельный словарь колонок для формата выгрузки Excel. Сводить его
с `COLUMNS` нельзя: в v1 «дата поставки» — это дата расчётов, а в v2 поставка
бумаг и оплата деньгами разнесены по разным парам колонок, каждая с плановой и
фактической датой. Общий словарь делал бы каждую новую формулировку риском для
уже работающего разбора. `resolve_columns(table, columns)` принимает словарь
параметром, `is_known_column` — тот же словарь в роли предиката для нарезки.
...
### `broker/sections.py` — стадия 1.5

```python
split_sections(tables, *, known_header) -> list[ReportSection]
```

Режет плоскую выгрузку Excel на секции. Реальный отчёт брокера — **одна таблица
на весь документ**: подписи разделов («5.1 Биржевые сделки…»), шапки колонок,
подзаголовки групп по бумагам и строки «Итого» лежат строками внутри неё.

`ReportSection` отдаёт секцию обычной `RawTable`, поэтому `resolve_columns` и
`as_dicts` работают дальше без изменений, плюс `groups` — подзаголовок группы для
каждой строки: в разделах сделок инструмент стоит не колонкой, а строкой над
своей группой сделок.

Правила распознавания строк и почему они такие:

| Строка | Признак | Почему не проще |
|---|---|---|
| подпись раздела | первая ячейка начинается с номера `N[.N[.N]]`, в строке нет чисел, в хвосте есть буква | требование «ровно одна ячейка» теряет «4. Оценка активов», у которой в той же строке стоят подписи колонок; без проверки на буквы сумма `1 234.56` в одиночной ячейке читается как подпись раздела №1 |
| шапка | ≥2 опознанных заголовка и ни одного числа | порог в три отсекал бы шапки, где половина колонок незнакома |
| второй ярус шапки | идёт сразу за шапкой, ≥2 заполненных ячеек, чисел нет | ровно этим он отличается от подзаголовка группы, у которого ячейка одна |
| подзаголовок группы | одна заполненная ячейка без чисел | |

Новая шапка внутри того же раздела начинает **новую** `ReportSection` с тем же
номером: в разделе 1 таких таблиц несколько подряд, и склеивать их нельзя.

### `broker/mapping.py` — выбор версии

```python
parse(content) -> ParsedReport
```

Достаёт таблицы один раз и отдаёт их той версии разбора, которая подходит:
наличие нумерованных разделов (`5.1`, `8.1.1`) означает выгрузку Excel — v2,
иначе v1. Признак берётся из документа, а не из даты отчёта или имени файла:
брокер менял выгрузку не в один день, а даты в имени файла может не быть вовсе.

Обе версии сохраняются рабочими (спека 4.1): архив за десять лет содержит файлы
обоих форматов, и номер применённой версии пишется в `raw_reports.parser_version`.

### `broker/mapping_v2.py` — стадия 2 для выгрузки Excel

Разбирает формат реального архива. Разделы и их роль:

| Раздел | Что даёт |
|---|---|
| `1`, `1.1` | остаток денег: строка «Исходящий остаток (всего)», не плановый (A-25) |
| `2` | остатки бумаг и справочник «наименование + регномер → ISIN» |
| `5.1`, `5.10` | сделки; пересечение разделов снимает ключ идемпотентности (A-22) |
| `5.4`, `5.9` | пропускаются осознанно, списком `SKIPPED_SECTIONS` с причиной (A-24) |
...
Три места, где разбор нетривиален:

1. **бумага сделки** (A-23). В строке сделки её нет; подзаголовок группы — одна
   склеенная строка «эмитент наименование регномер валюта» без ISIN. Поэтому
   раздел 2 разбирается первым и служит справочником, а в подзаголовке ищется
   самый длинный известный токен. Не нашлось — строка уходит в нераспознанные с
   текстом подзаголовка: событие с количеством, но без бумаги, нарушает
   инварианты журнала;
2. **бумага купона и налога**. В разделе 8.1 колонки бумаги нет — она названа в
   комментарии («Погашение купона № ОФЗ 26238»). Ищется ISIN, затем известное
   наименование. Не нашлось — операция всё равно записывается: деньги от этого не
   меняются, а сверка денег важнее привязки к бумаге;
3. **хвост раздела сделок**. После итогов идут обороты по площадкам в тех же
   колонках: «Московская биржа | 1 234.56» разбиралось бы как сделка с видом
   «1 234.56». Отсекается по подзаголовку группы («Оборот за отчетный период»).

Знак количества в разделе 8.2 берётся из отчёта только у конвертации: списание и
зачисление идут двумя строками, и минус — единственное, что их различает. У ввода
и погашения знак задан типом операции.
...
Повтор ключа **внутри одной пачки** схлопывается так же, как повтор с журналом:
одна и та же сделка стоит и в разделе заключённых сделок, и в разделе исполнения
обязательств (A-22), и второе вхождение — то же событие, а не второе. Без этого
импорт падал бы на UNIQUE-индексе при первом же реальном отчёте.
...
```python
init_db(settings=None) -> InitDbResult
```

Создаёт недостающие таблицы из метаданных `models.py` — для локального прогона
на SQLite, где миграции неприменимы: в SQLite нет ни `ALTER … DROP CONSTRAINT`,
ни типов из миграции 0001.

Для PostgreSQL поднимает `ValueError` и указывает на `alembic upgrade head`.
Второго пути к боевой схеме быть не должно: без миграций боевая БД и история её
изменений разъезжаются с первого же выпуска. Единственный источник правды при
этом один — метаданные `models.py`; миграции лишь переносят их изменения.

Существующие таблицы не трогаются и не удаляются: команда идемпотентна.
...
402 теста (из них 334 по этапу 1 и вспомогательным скриптам), без сети и без поднятого PostgreSQL. Тестовая БД — SQLite во
...
| `test_cli.py` | коды возврата: ноль после каждого месяца, ненулевой при расхождении, `2` при битом CSV; `init-db` создаёт схему на SQLite, идемпотентна и отказывается работать с PostgreSQL |
| `test_broker_v2.py` | формат выгрузки Excel: выбор версии по форме документа; нарезка плоской таблицы на разделы; бумага из подзаголовка группы; фактическая дата оплаты из двухъярусной шапки; НКД сверх суммы сделки; комиссии по видам; нулевой маркер погашения; списание бумаги разделом 8.2; остаток денег «всего», а не плановый; импорт и сверка сходятся end-to-end |
...
ранний формат в CP1251, `report_v2_2024-03.html` — выгрузка Excel одной таблицей
(формат реального архива: нумерованные разделы, двухъярусные шапки, подзаголовки
групп, повтор сделки в разделе 5.10). Все четыре синтетические; подробности и план замены — в
...
## 11. Состояние после бэкфилла

**Этап 1 завершён.** Весь архив импортирован в хронологическом порядке, `check`
возвращает ноль после каждого месяца, нераспознанных строк нет ни в одном
отчёте (импорт считает отчёт успешным только при пустом списке, поэтому
прохождение архива и есть доказательство).

### Что изменил прогон

Семь находок, каждая с допущением в `ASSUMPTIONS.md`. Две потребовали миграций:

| Находка | Правка |
|---|---|
| `TRANSFER` не входил в `_QUANTITY_EVENTS` — «Ввод/Вывод ЦБ» попадали в журнал, но проекция позиций их не видела | `domain/positions.py` |
| первый токен подзаголовка группы сделок — внутренний код выпуска формы ISIN (`MC` плюс десять цифр) | `mapping_v2._instrument_of`, A-23 |
| вознаграждение за заём бумаг — сумма в рублях в колонке «% по сделке» строки возврата | раздел `5.4` разбирается, тип `LENDING_INCOME`, миграция `0003`, A-21/A-24 |
| контрольное количество бумаг — «Плановое количество ЦБ» | `mapping_v2._security_balances`, A-26 |
| копейки в месяцах с займом | мягкий допуск `RECONCILE_LOAN_TOLERANCE`, A-27 |
| `UNIQUE(ticker)`: наименование не определяет бумагу | `models.py`, `domain/instruments.py`, миграция `0004`, A-28 |
| у списываемой стороны конвертации ISIN не напечатан | восстановление из комментария, A-29 |

Допущения A-04 и A-07 закрыты **прямой проверкой на отчёте**: сумма сделки,
делённая на `количество × цена ÷ 100`, дала круглый номинал (НКД снаружи), а
выплата купона совпала с `количество × размер купона` (выплата брутто).
Сходимость архива подтверждает их вторым, независимым способом.

Побочная находка той же проверки: **налога по купонам в разделе 8.1.1 нет**.
Удержание происходит не в момент выплаты, поэтому попарной связи «купон ↔ его
налог» в отчёте не существует — это ограничение для налогового контура этапа 3,
а не для журнала.

### Ограничения, оставшиеся осознанно

* **`predecessor_id` пуст.** Связь старого и нового выпуска при конвертации в
  журнал не пишется: перенос цены приобретения относится к налоговому контуру
  этапа 3. На количества это не влияет — списание и зачисление сходятся сами
  (A-29);
* **налог не привязан к выплате.** У купонов строки налога в отчёте нет вовсе, у
  дивидендов она своя и с выплатой общих идентификаторов не имеет. Журналу это
  не мешает (суммы брутто, налог отдельным событием), а этап 3 должен строить
  налоговую базу по периоду, а не по парам (A-04);
* **мягкий допуск на займах** (A-27) принимает до рубля расхождения по деньгам в
  отчётах с разделом `5.4`. Расхождение не исчезает: печатается и попадает во
  «Входящие». Опасность здесь в накоплении, а не в величине — остаток
  накопителен, поэтому смотреть надо на рост разницы от месяца к месяцу;
* **на PostgreSQL не запускалось.** Миграции `0002`–`0004` меняют CHECK-ограничения
  и уникальность, а на SQLite такие миграции неприменимы (`init-db` собирает схему
  из метаданных). Первый `alembic upgrade head` на настоящей БД стоит считать
  частью проверки;
* фикстура `report_v2_2024-03.html` повторяет раскладку реального отчёта, но
  числа в ней придуманы: она проверяет разбор и сверку, а не конвенции брокера.
...
(и `bonds.py` параллельного трека).
