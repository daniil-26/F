...
372 теста (из них 304 по этапу 1 и вспомогательным скриптам), без сети и без поднятого PostgreSQL. Тестовая БД — SQLite во
...
**Шаг 0 выполнен.** Форма реального отчёта разобрана дважды
([`BROKER-REPORT-FORMAT.md`](BROKER-REPORT-FORMAT.md)): по 15 обезличенным
отчётам 2017–2020 и по выжимке с 14 необезличенных отчётов 2024–2026. Формат
оказался выгрузкой Excel — весь отчёт одной таблицей, разделы строками внутри
неё — и за девять лет не изменился. Под него написаны `sections.py` (нарезка) и
`mapping_v2.py` (разбор); версия выбирается по форме документа в `mapping.py`.

Словарь операций собран целиком, новых типов вторая разведка не показала.

Что осталось:

* **суммы**. Обезличенный архив обнуляет числа, поэтому по нему нельзя было
  проверить, входит ли НКД в сумму сделки (A-07), брутто или нетто приходит
  купон (A-04) и где именно приходит вознаграждение за заём бумаг (A-21, A-24).
  Третий вопрос **закрыт прогоном по реальному архиву**: «% по сделке» строки
  возврата — сумма вознаграждения, и она пишется типом `LENDING_INCOME`
  (миграция 0003). Первые два по-прежнему ждут месяца, где проявятся;
* **T11 (бэкфилл)** сделан наполовину: механика прогона каталога есть
  (`import-broker <каталог>` в хронологическом порядке), самого прогона по
  реальному архиву — нет;
...
в порядке вероятности: бумага сделки, не найденная в разделе 2 (A-23);
неучтённый тип комиссии; операции последнего торгового дня месяца из-за T+1;
корпоративные действия помимо конвертации; купон нетто там, где ожидался брутто;
неполные начальные остатки.

Правки локализованы: формулировки колонок — в `anchors.py` (`COLUMNS` для v1,
`COLUMNS_V2` для v2), правила разбора — в `mapping_v1.py` и `mapping_v2.py`,
предметные решения — в `ASSUMPTIONS.md`. Третий формат разбирается добавлением
`mapping_v3.py` рядом и одной строкой в `mapping.py`: механизм версионирования
заложен в `raw_reports.parser_version`.
