...
| T11 Бэкфилл (частично) | прогон каталога с отчётами в хронологическом порядке, с остановкой на первом расхождении | `jobs/import_broker.import_broker_archive`, `adapters/files.py` |

Не выполнено: шаг 0 (разведка архива отчётов) и вторая половина T11 — прогон по
реальному архиву и разбор того, что он покажет. Обе задачи требуют настоящих
отчётов брокера. Следствия описаны в разделе 11.
...
|---|---|---|
| `import-broker <путь…> [--dry-run] [--account] [--force] [-r] [--pattern] [--keep-going]` | `jobs.import_broker.import_broker_archive` | `1`, если сверка не сошлась или есть нераспознанные строки; `2`, если отчётов не найдено |
...
### `files.py` — отбор файлов

```python
discover_reports(paths, *, recursive=False, pattern=None,
                 skip_suffixes=()) -> DiscoveredFiles
```

Разворачивает список путей: файл берётся как есть, каталог — по содержимому.
Каталог фильтруется по `REPORT_SUFFIXES = (".html", ".htm", ".xls")` — брокер
отдаёт ту же HTML-таблицу и под расширением `.xls`. Явная `--pattern` отменяет
фильтр по расширению: если маска задана, пользователь уже сказал, что берёт.

`DiscoveredFiles.notes` — заметки о пропущенном («файла нет», «отчётов не
найдено»). Они возвращаются, а не печатаются: молча проигнорированный аргумент
хуже лишней строки в выводе, но решать, куда её писать, — дело вызывающего.

Знание о том, как разложен архив, — такой же внешний мир, как HTML, поэтому
живёт в адаптерах. `tools/_report_grid.iter_report_files` — тонкая обёртка над
этой же функцией: иначе вспомогательные скрипты смотрели бы на один набор
файлов, а импорт брал другой.
...
```python
import_broker_archive(paths, *, account_code=None, dry_run=False, force=False,
                      recursive=False, pattern=None, stop_on_error=True,
                      settings=None) -> ArchiveImportResult
```

Оболочка над `import_broker_report` для каталога с отчётами (STAGE-1, T11).
Каждый отчёт по-прежнему импортируется **своей** транзакцией: прогон за десять
лет не должен откатываться целиком из-за расхождения в последнем месяце.

Порядок — `_in_chronological_order(paths)`: период читается разбором самого
файла, а не из его имени. Имена в архиве бывают вида `report (12).html`, а
позиции и остатки накопительны — отчёт за март, применённый после майского, даст
верный итог и неверную историю. Файлы, у которых период не прочитался, идут в
конец по имени: там они и отчитаются об ошибке.

`stop_on_error=True` (по умолчанию) останавливает прогон на первом отчёте,
который не сошёлся или не разобрался. Импорт следующего месяца поверх
несошедшегося предыдущего накапливает расхождение и запутывает разбор. Флаг
`--keep-going` снимает остановку — это режим разбора, а не загрузки.

`ArchiveImportResult`: `results` (в порядке импорта), `failures` — пары
`(путь, текст ошибки)` для файлов, на которых импорт поднял исключение, `notes`
от `discover_reports`, `stopped_early` — прогон оборван и часть каталога не
тронута. Свойства: `ok` (нет ни падений, ни неудачных отчётов) и `committed` —
сколько отчётов подтверждено.
...
341 тест (из них 273 по этапу 1 и вспомогательным скриптам), без сети и без поднятого PostgreSQL. Тестовая БД — SQLite во
...
| `test_import_archive.py` | каталог импортируется в хронологическом порядке по периоду отчёта, а не по имени файла; остановка на первом несошедшемся отчёте и `--keep-going`; битый файл попадает в `failures`, а не рушит прогон; `-r`, `--pattern`, отбор по расширению; `2` на пустом каталоге; вывод для одного файла не изменился |
...
* допущения A-01, A-02, A-03, A-04, A-07, A-08, A-10 помечены как «требует
  подтверждения»;
* T11 (бэкфилл архива) сделан наполовину: механика прогона каталога есть
  (`import-broker <каталог>`), самого прогона по реальному архиву — нет.
